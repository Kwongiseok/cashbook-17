...
class MainView extends Component {
  ...
  getTotalElement() {
    ...
    return this.state.cashbooks.filter(cashbook => cashbook.categoryType === 'income' && incomeChecked || cashbook.categoryType === 'expenditure' && expenditureChecked).length;
  }
  ...
  registerCashbookToList($list) {
    ...
    for (let i = 31; i >= 1; i--) {
      const cashbooks = this.state.cashbooks.filter(cashbook => cashbook.date === `${year}-${month}-${i}`).filter(cashbook => cashbook.categoryType === 'income' && incomeChecked || cashbook.categoryType === 'expenditure' && expenditureChecked);
      ...
      cashbooks.forEach(cashbook => {
        ...
        $cashbookItem.addEventListener('click', e => {
          triggerByElement(document.querySelector('.input-bar'), 'selectCashbook', {
            ...
            price: cashbook.price * (cashbook.categoryType === 'expenditure' ? -1 : 1),
            memo: cashbook.memo,
...
class BarChart extends Component {
  ...
  mounted() {
    const $container = document.querySelector('.bar-container');
    $container.innerHTML = this.makeBarGraph(BarChart_dummy);
...
class DonutChart extends Component {
  ...
  mounted() {
    ...
    this.appendCircle($svg, DonutChart_dummy);
